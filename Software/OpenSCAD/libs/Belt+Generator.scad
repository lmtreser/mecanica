 ...
 *
 * Description:
 * This OpenSCAD script generates fully parametric timing belts with
 * customizable profiles, dimensions, and tooth configurations. It
 * supports various industry-standard belt types, including MXL, XL,
 * L, GT2, HTD, T-series, and more.
 *
 * License:
...
// Number of teeth in the belt.
Teeth = 36;

// Width of the belt.
Width = 6;

// Path shape of the belt.
Shape = "loop"; // [loop: Loop, straight: Straight] 

// Select which sides have teeth.
Sides = "inside"; // [inside: Inside, outside: Outside, both: Both sides]
...
// Pitch value (distance between teeth).
Pitch = 4;

// Override the thickness?
Backing_Override = false;
...
/**
 * Creates a parametric toothed belt.
 */
module toothed_belt(
    type = Type,
    teeth = Teeth,
    width = Width,
    backing = Backing_Override ? Backing : undef,
    pitch = Pitch_Override ? Pitch : undef,
) {
    profile = _belt_tooth_profile(type);
    data = _belt_tooth_profile_data[search([type], _belt_tooth_profile_data)[0]];
    pitch = is_num(pitch) && pitch > 0 ? pitch : data[1];
    backing = is_num(backing) && backing > 0 ? backing : data[2];
    plo = data[3];
    width = is_num(width) && width > 0 ? width : data[4];
    
    if (Shape == "straight") {
        _belt_straight(teeth, pitch, backing, profile, width);  
    } else {
        _belt_loop(teeth, pitch, plo, backing, profile, width);  
    }
}

/**
 * Generates a closed-loop timing belt.
 *
 * @param teeth     Number of teeth in the belt.
 * @param pitch     Distance between adjacent teeth.
 * @param plo       Pitch line offset (distance from pitch line to belt root).
 * @param backing   Thickness of the belt backing (material behind the teeth).
 * @param profile   2D tooth profile shape.
 * @param width     Total width of the belt.
 */
module _belt_loop(teeth, pitch, plo, backing, profile, width) {
    tooth_bounds = pointlist_bounds(profile);
    tooth_width = tooth_bounds[1].x - tooth_bounds[0].x;
    tooth_height = tooth_bounds[1].y;
    
    pitch_diameter = (teeth * pitch) / PI;
    // root_diameter = pitch_diameter - (plo * 2);
    // Need to account for profiles without known PLO as well:
    root_diameter = root_diameter(Type, teeth, pitch);
    outer_diameter = root_diameter + (backing * 2);
    tooth_base_radius = sqrt(pow(root_diameter/2, 2) - pow((tooth_width)/2, 2));
    
    color(Color)
    linear_extrude(height = width) {
        // Backing
        ring(d=root_diameter, ring_width=backing);

        // Teeth
        for (i = [0:teeth-1]) {
            if (Sides != "outside") 
                zrot(i/teeth*360) fwd(tooth_base_radius) polygon(profile);
            if (Sides != "inside") 
                zrot(i/teeth*360) back(-tooth_base_radius-backing) zrot(180) polygon(profile);
        }
    }
    
    // Debug info
    if (Debug) {
        fwd(root_diameter / 2 + 6)
        belt_info([
            ["Length", teeth * pitch],
            ["Width", width],
            ["Backing", backing],
...
/**
 * Generates a straight (open) timing belt.
 *
 * @param teeth     Number of teeth in the belt.
 * @param pitch     Distance between adjacent teeth.
 * @param backing   Thickness of the belt backing (material behind the teeth).
 ...
 * @param width     Total width of the belt.
 */
module _belt_straight(teeth, pitch, backing, profile, width) {
    bounds = pointlist_bounds(profile);
    tooth_width = bounds[1].x - bounds[0].x;
    length = teeth * pitch;
    ...
    linear_extrude(height = width) {
        ...
        // Teeth
        left((teeth * pitch) / 2 - tooth_width / 2 - tooth_width / 4) {
            for (i = [0:teeth-1]) {
                if (Sides != "outside") 
                    translate([i * pitch, backing/2]) polygon(profile);
                if (Sides != "inside") 
                    translate([i * pitch, -backing/2]) zrot(180) polygon(profile);
            }
        }
    }
...
/**
 * Returns the root diameter of the belt.
 */
function root_diameter(type, teeth, pitch) =
    ( type == "MXL" ) ?
		_root_diam_plo( teeth, pitch, 0.254 )
    : ( type == "40DP" ) ?
        _root_diam_plo( teeth, pitch, 0.1778 )
    : ( type == "XL" ) ?
        _root_diam_plo( teeth, pitch, 0.254 )
    : ( type == "L" ) ?
        _root_diam_plo( teeth, pitch, 0.381 )
    : ( type == "T2.5" ) ?
        _root_diam_curvefit( teeth, 0.7467,0.796, 1.026 )
    : ( type == "T5" ) ?
        _root_diam_curvefit( teeth, 0.6523, 1.591, 1.064 )
    : ( type == "T10" ) ?
        _root_diam_plo( teeth, pitch, 0.93 )
    : ( type == "AT5" ) ?
        _root_diam_curvefit( teeth, 0.6523, 1.591, 1.064 )
    : ( type == "HTD_3mm" ) ?
        _root_diam_plo( teeth, pitch, 0.381 )
    : ( type == "HTD_5mm" ) ?
        _root_diam_plo( teeth, pitch, 0.5715 )
    : ( type == "HTD_8mm" ) ?
        _root_diam_plo( teeth, pitch, 0.6858 )
    : ( type == "GT2_2mm" ) ?
        _root_diam_plo( teeth, pitch, 0.254 )
    : ( type == "GT2_3mm" ) ?
        _root_diam_plo( teeth, pitch, 0.381 )   
...
function _root_diam_plo( teeth, tooth_pitch, pitch_line_offset ) 
...
/**
 * Specifications for different tooth profiles.
 *
 * Reference data from sdp-si.com and th.misumi-ec.com.
 */
_belt_tooth_profile_data = [
    // type,        pitch,  backing,    plo         default belt width
	[ "MXL",        2.032,  0.64,       0.254,      6.35 ],
    [ "XL",         5.08,   1.03,       0.254,      7.94 ],
	[ "L",          9.525,  1.66,       0.381,      19.05 ],
    [ "40DP",       2.073,  0.74,       0.1778,     4.7625 ],
	[ "T2.5",       2.5,    0.6,        undef,      6 ],
	[ "T5",         5,      1,          undef,      10 ],
	[ "T10",        10,     2,          0.93,       16 ],
	[ "GT2_2mm",    2,      0.76,       0.254,      6 ],
	[ "GT2_3mm",    3,      1.27,       0.381,      9 ],    
	[ "GT2_5mm",    5,      1.88,       0.5715,     15 ],
	[ "HTD_3mm",    3,      1.19,       0.381,      9 ],
	[ "HTD_5mm",    5,      1.73,       0.5715,     9 ],
	[ "HTD_8mm",    8,      2.64,       0.6858,     30 ],
    [ "AT5",        5,      1,          undef,      10 ],
];

/**
 * Returns the tooth profile based on the type.
 *
 * Credits go to 'droftarts' for extracting these vectors.
 */
