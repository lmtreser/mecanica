...
// Number of set screws (90° apart).
Hub_Screw_Count = 1; // [0:1:4]

// Type of set screws.
Hub_Screw_Type = "M4"; // [M3, M4, M5, M6, M8, M10, M12, M14, M16, M18, M20]

// Add screw thread?
Hub_Screw_Thread = true;
 ...
 */
module _pulley_bore_mask(height, type = Bore_Type, size = Bore_Size, chamfer = Bore_Chamfer) {
    h = height + Shiftout;
    ...
    up(Shiftout / 2) {
        if (type == "cylinder") {
            ...
            keyway = keyway_size(size);
            backing = size / 2;
            cyl(d = size, h = h, chamfer = -chamfer);
            zrot(90)
                back(size / 2 + keyway.y / 2 - backing / 2) 
                    cube([keyway.x, keyway.y + backing, h], center = true);
        } else if (type == "D") {
...
module _pulley_hub(
    ...
    screw_type = Hub_Screw_Type,
    screw_thread = Hub_Screw_Thread,
    screw_count = Hub_Screw_Count,
    screw_offset = Hub_Height / 2,
    anchor = TOP,
    spin = 0,
    orient = UP,
) {
